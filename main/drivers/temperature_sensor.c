 ...
 * @brief   MCP9808 temperature sensor I2C helpers implementation
 * @author  Max Bucks
 * @date    2026-01-14
 *****************************************************************************/
...
/**
 * @brief Initialize I2C master bus and attach MCP9808 devices.
 *
 * Configures the I2C master bus, creates the bus handle and initializes
 * both central and exhaust MCP9808 sensor device handles.
 */
void i2c_init(void) {
...
/**
 * @brief Initialize a single MCP9808 device on the bus.
 *
 * Adds the device to the I2C master bus, writes a default config and
 * reads back manufacturer/device IDs for verification.
 */
esp_err_t mcp9808_init(i2c_master_bus_handle_t bus, uint8_t address,
    ...
    ESP_LOGI(TAG, "Config readback @0x%02X = 0x%02X 0x%02X", address, config_read[0],
             config_read[1]);
    ...
    if (manuf_id != MCP9809_MANUFACTURER_ID || (dev_id >> 8) != MCP9808_DEVICE_ID) {
        ESP_LOGW(TAG,
                 "Device at 0x%02X does not match expected MCP9808 IDs (Got MID=0x%04X DID=0x%04X)",
                 address, manuf_id, dev_id);
    } else {
        ESP_LOGI(TAG, "Verified MCP9808 at 0x%02X (MID=0x%04X DID=0x%04X)", address, manuf_id,
                 dev_id);
    }
...
/**
 * @brief Read and convert temperature register to degrees Celsius.
 *
 * Returns -1000.0f on error to indicate failure.
 */
...
/**
 * @brief Scan I2C bus for devices and log found addresses.
 */
...
/**
 * @brief Return the I2C master bus handle created by 'i2c_init()'.
 */
i2c_master_bus_handle_t get_i2c_bus_handle(void) { return bus_handle; }
