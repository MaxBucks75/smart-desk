/******************************************************************************
 * @file    fingerprint.c
 * @brief   R503 fingerprint sensor implementation for Smart Desk
 * @author  Max Bucks
 * @date    2026-01-14
 *****************************************************************************/
...
// Allocate buffer size of 64 bytes for ~16 byte UART messages
const int         R503_buffer_size = (512 * 2);

// Holds the data from the last R503 rx transmission
static uint8_t R503_rx_buf[MAX_R503_PACKET_SIZE];

// Holds the index for the number of bytes in the last rx transmission
...
void R503_init(void) {
    ...
    // Install UART driver on port 2, TX size 32 bytes, RX size of 32 bytes, event queue of 10
    // stored in R503 queue, and 0 interrupt flags.
    ...
    // Start the UART task to poll for response
    xTaskCreate(uart_event_task, "uart_event_task", 4096, NULL, PRIO_FINGERPRINT,
                NULL);
...
/**
 * @brief UART event task that collects incoming R503 packets.
 *
 * This FreeRTOS task waits for UART events, assembles incoming bytes into
 * 'R503_rx_buf', and signals the response queue when a complete packet
 * has been received.
 */
    ...
    while (1) {
        ...
        if (xQueueReceive(uart_queue, &event, portMAX_DELAY) == pdTRUE) {
            ...
            if (event.type == UART_DATA) {
                ...
                int length = uart_read_bytes(uart_num, (R503_rx_buf + R503_rx_index), event.size,
                                             portMAX_DELAY);
                ...
                if (R503_rx_index >= 9) {
                    ...
                    // Get the total length of the ack packet (data from word at index 7, plus the 9
                    // non-payload bytes)
                    ...
                    if (R503_rx_index >= total_length) {
                        ...
                        // Reset local index variable
                        R503_rx_index        = 0;
                        // Notify command task we done!
                        xQueueOverwrite(response_queue, &packet_length); 
                    }
                }
            }
        }
...
/**
 * @brief Wait for a finger to be presented and capture its image.
 *
 * Repeatedly attempts to 'generate_image()' until a valid image is
 * captured or the provided timeout elapses. On success it converts the
 * image into a character file in the specified buffer.
 */
...
esp_err_t delete_fingerprint(uint16_t location_id, uint16_t count) {
    ...
    uint8_t cmd[5] = {R503_DELETE_TEMPLATE, (location_id >> 8) & 0xFF, location_id & 0xFF,
...
/**
 * @brief Load a character file from R503 library into a buffer
 *
 * @param buffer_id buffer character file is stored into
 * @param location_id character file id in R503 library
 * @return esp_err_t ESP_OK if successful
 */
esp_err_t load_char(uint8_t buffer_id, uint16_t location_id) {

    uint8_t cmd[4] = {R503_LOAD_CHAR, buffer_id, (location_id >> 8) & 0xFF, location_id & 0xFF};

    return send_package_and_read_ack(cmd, 4);
}

/**
 * @brief Read then sort through the R503 status register
 */
esp_err_t read_system_parameters(void) {
    ...
    uint32_t device_address   = ((uint32_t)R503_rx_buf[18] << 24 | (uint32_t)R503_rx_buf[19] << 16 |
                               (uint32_t)R503_rx_buf[20] << 8 | R503_rx_buf[21]);
...
/**
 * @brief search the R503 library for the first free id.
 *
 * @return uint16_t id of the first free id in the R503 library or -1 if library is full.
 */
...
/**
 * @brief Perform a full enrollment sequence for a single finger.
 *
 * Guides the user through two captures, verifies the two images match,
 * then stores the resulting template in the sensor library.
 */
esp_err_t auto_enroll(void) {
    ...
    int iterations = 0;
    // Continuously generate image until a finger isn't recognized or we've tried generating an
    // image 10 times
    do {
...
/**
 * @brief Attempt to identify the presented finger against the stored library.
 *
 * Captures an image, creates a character file and issues a 'SEARCH'
 * command to the sensor. Returns 'ESP_OK' on a successful match.
 */
esp_err_t auto_identify(void) {
    ...
    if (err == ESP_ERR_NOT_FOUND) {
        ...
        err = generate_char_file_from_image(
            R503_CHAR_BUFFER_1); // Collect finger and store in char buffer one
    }
    ...
    if (err == ESP_ERR_INVALID_STATE) {
        ...
        uint8_t cmd[6] = {
            R503_SEARCH, R503_CHAR_BUFFER_1,
        ...
        if (R503_rx_buf[9] == R503_SUCCESS) {
            ...
            ESP_LOGI(TAG, "Fingerprint match found at page ID: 0x%04X, with match score: 0x%04X",
                     page_id, match_score);
            return ESP_OK;
